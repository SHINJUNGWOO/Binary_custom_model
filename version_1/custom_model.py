...
class Aresnet_2(nn.Module):
    def __init__(self,in_channel,out_channel,stride = 1):
        super(Aresnet_2,self).__init__()
        self.stride = stride
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        self.move1_1 = Learnablebias(in_channel)
        self.bin3x3 = BinaryConv(in_channel, in_channel, stride=stride, kernel_size=3,padding=1)
        self.bn1 = norm_layer(in_channel)
        ...
        if in_channel == out_channel:
            ...
            self.grouped_conv = GroupedConv(in_channel, out_channel, stride=1, kernel_size=1,padding=0,groups=2)
            self.bn2_1 = norm_layer(in_channel)
            self.bn2_2 = norm_layer(in_channel)
            self.bn2_3 = norm_layer(in_channel)
            self.bn2_4 = norm_layer(in_channel)
    ...
    def forward(self,x):
        ...
        if self.in_channel == self.out_channel:
            ...
            assert self.out_channel == self.in_channel *2
            x_2_res = channel_shuffle(x_2)
            x_2_res = self.grouped_conv(x_2_res)

            x_2_1 = self.bn2_1(x_2_res[:,:self.out_channel//2,:,:])
            x_2_2 = self.bn2_2(x_2_res[:,self.out_channel//2:,:,:])

            # x_2_1 = self.bn2_1(x_2_res[:,:self.out_channel//4,:,:])
            # x_2_2 = self.bn2_2(x_2_res[:,1*self.out_channel//4:2*self.out_channel//4,:,:])
            # x_2_3 = self.bn2_3(x_2_res[:,2*self.out_channel//4:3*self.out_channel//4,:,:])
            # x_2_4 = self.bn2_4(x_2_res[:,3*self.out_channel//4:,:,:])
            ...
            #x_2 = torch.cat([x_2_1, x_2_2, x_2_3, x_2_4], dim=1)
...
class Aresnet_3(nn.Module):
    def __init__(self,in_channel,out_channel,stride = 1):
        super(Aresnet_3,self).__init__()
        self.stride = stride
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        self.move1_1 = Learnablebias(in_channel)
        self.bin3x3 = GroupedConv(in_channel, in_channel, stride=stride, kernel_size=3,padding=1,groups=2)
        self.bn1 = norm_layer(in_channel)
...
class Aresnet_4(nn.Module):
    def __init__(self,in_channel,out_channel,stride = 1):
        super(Aresnet_4,self).__init__()
        self.stride = stride
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        self.move1_1 = Learnablebias(in_channel)
        self.bin3x3 = GroupedConv(in_channel, in_channel//2, stride=1, kernel_size=1,padding=0,groups=2)
        self.bn1 = norm_layer(in_channel)
        ...
        if in_channel == out_channel:
            ...
            self.grouped_conv = GroupedConv(in_channel, out_channel, stride=stride, kernel_size=3,padding=1,groups=2)
            self.bn2_1 = norm_layer(in_channel)
            self.bn2_2 = norm_layer(in_channel)
            self.bn2_3 = norm_layer(in_channel)
            self.bn2_4 = norm_layer(in_channel)
    ...
    def forward(self,x):
        ...
        x_1 = self.bin_act(x_1)
        x_1 = channel_shuffle(x_1)
        x_1_conv = self.bin3x3(x_1)
    
        x_1 = torch.cat([x_1_conv,x_1[:,self.in_channel//2:,:,:]],dim=1)
        x_1 = self.bn1(x_1)
...
class Aresnet_5(nn.Module):
    def __init__(self,in_channel,out_channel,stride = 1):
        super(Aresnet_5,self).__init__()
        self.stride = stride
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        if in_channel == out_channel:
            ...
            self.bin3x3 = GroupedConv(in_channel, in_channel*2, stride=stride, kernel_size=3, padding=1, groups=2)
            self.bn1 = norm_layer(in_channel*2)

            self.move1_2 = Learnablebias(in_channel*2)
            self.prelu1 = nn.PReLU(in_channel*2)
            self.move1_3 = Learnablebias(in_channel*2)

            self.move2_1 = Learnablebias(in_channel*2)



            self.grouped_conv = GroupedConv(in_channel*2, out_channel, stride=1, kernel_size=1,padding=0,groups=2)
            self.bn2_1 = norm_layer(in_channel*2)
            self.bn2_2 = norm_layer(in_channel*2)
            self.bn2_3 = norm_layer(in_channel*2)
            self.bn2_4 = norm_layer(in_channel*2)
    ...
    def forward(self,x):
        ...
        x_1 = self.bin_act(x_1)
        x_1 = channel_shuffle(x_1) if self.out_channel != self.in_channel else x_1
        x_1 = self.bin3x3(x_1)
        x_1 = self.bn1(x_1)

        if self.stride == 2:
            x = torch.cat([self.pooling(x)+x_1[:,:self.in_channel,:,:],self.pooling(x)+x_1[:,self.in_channel:,:,:]],dim=1)
        elif self.in_channel != self.out_channel:
            x = torch.cat([x+x_1[:,:self.in_channel,:,:],x+x_1[:,self.in_channel:,:,:]],dim =1)
        ...
        if self.in_channel == self.out_channel:
            ...
            assert self.out_channel == self.in_channel *4
            x_2_res = channel_shuffle(x_2)
            x_2_res = self.grouped_conv(x_2_res)
