...
from pretrained import cifar_resnet56
...
parser.add_argument('--save', type=str, default='./custom_models_inv/ver_3', help='path for saving trained models')
parser.add_argument('--data',default='../img_data/cifar100', metavar='DIR', help='path to dataset')
...
def main():
    ...
    #optimizer = torch.optim.SGD(model_student.parameters(), 0.1, momentum=0.9, weight_decay=0.00001)
   ...
   # traindir = os.path.join(args.data, 'train')
    ...
    cifar100_train = datasets.CIFAR100(args.data, train=True, download=True,
    ...
    cifar100_eval = datasets.CIFAR100(args.data, train=False, download=False,transform=transforms.Compose([transforms.ToTensor(),
                                        transforms.Normalize(
                                               mean=[0.5071, 0.4865, 0.4409],
                                               std=[0.2009, 0.1984, 0.2023],
                                           ),]))
    train_loader = DataLoader(cifar100_train, batch_size=args.batch_size, shuffle=True, )
    val_loader = DataLoader(cifar100_eval, batch_size=args.batch_size, shuffle=True, )
