...
#{{{
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1, padding=1,groups =2,binarized = False):
        ...
        self.bn       = nn.ModuleList([nn.BatchNorm2d(self.group_out_channel) for i in range(groups)])
    ...
    def forward(self,x):
        ...
        out = torch.chunk(out,self.groups,1)

        out = torch.cat([self.active[i](self.bn[i](self.leanable[i](piece )))\
                         for i,piece in enumerate(out)],dim=1)
        return out
#}}}
